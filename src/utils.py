...
"""
def load_checkpoint(model, optimizer, config, load_last, strict_config=True):
  checkpoint_path = config['checkpoint_dir'] + config['exp_name'] + ("_last_model.pt" if load_last else "_best_model.pt")
